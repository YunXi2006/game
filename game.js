...
const ITEM_DESCRIPTIONS = {
  ...
  '告示碎片':
    '一張印有校園設施資訊的碎紙，其中一行讓李白停住了目光——「古典文獻閱覽室：本週展出孤本《太白遺卷》，記李白晚年出走之所悟。」命運的線索，從這張無人在意的紙開始。',
  '子時啟陣密語':
    '從《太白遺卷》封底隱字辨認出的啟陣條件：「子時三更，月滿中庭，以詩換門，以酒啟陣——啟陣之前，須解城居者未之知之真意，方可入門。」子時、月圓、詩、酒，以及那道關乎「知」的考驗——缺一不可。',
  '時輪陣圖草稿':
    '白髮老先生研究三十年的手繪草圖，描繪時輪陣的完整結構。啟動條件：古水之源、子時月滿、以酒為媒，並須由原作者親自吟誦自己的詩——帶著真情，不能只是背稿。另附注：陣法啟動前有最後一問，考驗旅者是否真正走出了城居之境。',
  '教授研究報告摘要':
    '古典文學教授的田野調查節錄。確認清月井舊址位於校園東北角，唐代地下水脈殘存，現為石板廣場。另記：啟動詩句須由原作者吟誦，帶著真情，陣法才能感應氣場共鳴。最後一關：需回答袁宏道《滿井遊記》末句「城居者未之知也」中「知」字的真義——親身感受，而非頭腦知識。',
  '清月井路線':
...
function resolveText(sceneId, scene) {
  ...
  if (sceneId === 'scene_5_3a') {
    return state.clue >= 3
      ? '吟唱聲在廣場上迴盪，藍光從石板縫隙中爆發，將整片廣場淹沒在白色的光芒裡。李白感到身體開始輕盈，像酒意上湧，像山風托舉，像月光把他一點一點地溶解。他在消失前的最後一刻，俯身在石台邊緣寫下幾個字——沒有筆，用指尖，蘸著殘餘的酒液：「今日方知，城外有春。」然後，白光吞沒了一切。'
      : '吟唱聲落下，白光乍現。他走了，帶著這一天走出城居之境所得到的一切，回到了他的時代。';
  }
  // scene_5_4：依是否有石台題字（clue >= 3 且走過 5_2a）決定尾聲細節
  if (sceneId === 'scene_5_4') {
    const hasInscription = state.clue >= 3 && state.favor >= 2;
    const inscriptionLine = hasInscription
      ? '細心的人或許會注意到石台邊緣有幾個淡淡的字跡，酒漬未乾——「今日方知，城外有春。」沒有人知道那是誰留下的，也沒有人知道「今日」是哪一天。'
      : '';
    return `翌日清晨，幾個早起的學生路過東北角廣場，發現石板縫隙中盛開了幾株無人認識的野花，淡黃色，帶著穀物與山泉混合的氣味。圖書館的《太白遺卷》封底，隱紋消失了。${inscriptionLine}只有風，還在從東北方吹來。`;
  }
